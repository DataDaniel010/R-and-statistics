...
-   **15.06.2024** Проблема множественных сравнений
...
-   **22.06.2024** Проблема множественных сравнений 2, дисперсионный анализ
...
df %>% select(X) %>% t.test()
```
Если p-value большое мы не можем отклонить нулевую гипотезу и не можем ее принять

## Разминка 2. Типы ошибок при тестировании гипотез

Какие два типа ошибок мы можем допустить при тестировании гипотез?
Первого и второго рода. Первого рода - отклонить верную нулевую гипотезу. Второго - не отклонить не верную нулевую гипотезу. 
...
```
Мы можем контролировать ошибку первого рода - выставляя тот порог с которым сравниваем (альфа), эта же альфа связана с границами ДИ (95 процетный интервал).
Ошибку второго рода не получится контролировать явно - можно через альфа - чем меньше альфа, тем больше вероятность ошибки второго рода. Косвенно через увеличение размера выборки. Чем больше выборка тем уже ДИ, тем с большей вероятностью обнаружим разницу. Power analysis - анализ силы, расчет объема выборки, количественное планирование эксперимента. 
...
-   Что такое 95% доверительный интервал?
    -   Это процедура.
    -   Процедура на основе данных (= выборки) генерирует интервал (границы интервала - случайные величины, которые рассчитаны на основе данных).
...
-   Бывают ли не 95% интервалы?
    -   Конечно, опять выбираем $\alpha$, вероятность покрытия тогда будет $1-\alpha$. Альфа может быть и 1 процент и 10.
...
-   А что тогда соответствует "ошибке второго рода"?
    -   В каком-то смысле длина интервала -- контролируем только косвенно, тоже "неопределенность" процедуры. При увеличении объема выборки интервал сужается.
...
-   Оценка параметра и неопределенность этой оценки aka доверительные интервалы (есть оценка и есть погрешность этой оценки)
...
-   Как именно мы выбираем $\alpha$? Почему 0,05?
    ...
    -   Выбор $\alpha$ внешний по отношению к математической статистике, диктуется экономическими (большая выборка стоит больше), этическими, эстетическими и прочими соображениями исследователя.
...
X и Y с друг другом коррелируют
...
t_stats <- tibble("X_t" = numeric(), "Y_t" = numeric()) #генератор случайных чисел
...
t_stats %>% 
  ...
  theme_classic()
```
Наши данные не независимы, ДИ не независимы
```{r}
...
```
На 18 тестах мы занимаемся условно подбрасыванием монетки - попадет или не попадет с вероятностью 0,5
На 42 тестах вероятность покрытия равна 0,1, с вероятностью 90% промахнемся. Это и есть проблема множественных сравнений. Если делаем множество тестов вероятность ошибки растет. 
FWER - вероятность осечки нашего квадра, вероятность того, что хотя бы один ДИ промахнется. 

## Итого

-   В нашем контексте под одним "сравнением" мы понимаем одну оценку или один статистический тест (или один ДИ)
-   На оценку многих параметров мы можем смотреть как на оценку одного многомерного параметра, *в нашем случае двумерного*
-   Аналогом доверительного интервала является доверительная область (ДО), *в нашем случае доверительный квадр*
...
- Логика будет та же самая, если сравниваем разные группы по одному параметру, а не одну группу по разным параметрам.
...
Интервалы, построенные с вероятностью покрытия $1 - \alpha_{adj}$: simultaneous CI (одновременные ДИ)

## FWER. Поправка Бонферрони. Пример 1

Пусть мы хотим построить $n$ интервалов с $FWER = \alpha$

Бонферрони предлагает: $CWER = \alpha_{adj} := \alpha/n$. Такая процедура гарантирует, что $FWER <= 0.05$ Причем неважно зависимые или нет интервалы
...
*Материал из [Википедии](https://ru.wikipedia.org/wiki/SF-36):*
...
Состояние человека описывается положением на восьми шкалах -- нас интересует точность оценки как целого, а не точность отдельных составных -- используем FWER. То есть имеем восьмимерное пространство. Каждая из осей - отдельная шкала. Интересует все 8 оценок вместе. 
...
-   Любая поправка снижает CWER. Какой у этого побочный эффект (снижаем альфу)?
...
*-- Не подскажете, где вход в метро?*
...
-   Оцениваем сложную систему, состоящую из разных частей
    ...
    -   Нам важно совместное поведение оценок, а не поведение отдельных оценок в среднем. *"Все или ничего"*
    -   Сравните с последовательным соединением элементов в цепи: чтобы сигнал прошел, отработать должен каждый из них
    - Соответствует логическому оператору И
...
Поправлять будем p-values, в результате получим *adjusted p-values*, $p_{adj}$.

$p^{adj}_i := n \cdot p_i$

Умножаем p-value на количество тестов. Не может быть больше 1 (если выходит больше, то его обрезают на этой единице)
...
Чтобы не делать руками есть функция p.adjust()
```{r}
...
Гипотеза была что мат ожидание Х = 0 И Y = 3 vs что хотя бы одно не верно. Если бы хоть одно из полученных p-value было меньше порога, то мы бы отвергли нулевую гипотезу и также знали бы по какой оси произошло отклонение или по каким осям. 
...
-   Поправка Холма однако часто более мягкая, чем поправка Бонферрони. Соответственно, тесты имеют большую силу. Не так сжимается альфа
...
-   Минус -- это нестатическая процедура, склеивает конкретные тесты вместе, нельзя разделить и пересобрать. Нужна вся пачка p-values, нельзя по отдельности их поправлять. 
...
-   Любая поправка снижает CWER. Какой у этого побочный эффект?
    -   Увеличение вероятности ошибки второго рода aka удлинение интервалов по отдельности.
    -   Конкретно Бонферрони очень консервативен. Т.е. для большого количества тестов $FWER << 0.05$, вероятности ошибки второго рода (длины интервалов) очень большие. Если смотрим на p-values лучше использовать поправку Холма. Если много тестов лучше использовать другие поправки (но у них есть свои критерии, которые можно будет не всегда проверить).
-   Существуют альтернативные поправки: Шидак, Симс, Шеффе (для регрессионных моделей и тестирования в регрессионных моделях), Хохберг (не путать с Бенджамини-Хохбергом!) и т.д.
    ...
    -   Бонферрони можно использовать для "склеивания" каких угодно тестов. Некоторые процедуры узкоспециальны (например, ANOVA и post-hoc процедуры к ней). Допустим 5 групп и проводим попарные тесты - можем склеить, используя Бонферрони. В одной группе измеряем несколько параметров, например, давление, температура, вес, если хочется, то можно собрать их в одну группу - склеить. Не обязательно это тесты среднего. Даже если между параметрами есть корреляция можно использовать поправку. Нет требования на независимость тестов и ДИ. Можно собрать вместе результат t-test, теста на равенство дисперсий и протестировать Бонферрони.
...
*Ожидаемая* доля *ложных* открытий от *всех* открытий называется False Discovery Rate (FDR). Мы можем контролировать FDR вместо FWER -- обычно это приводит к более мягким поправкам и, соответственно, более сильным тестам. Зачастую более разумно контролировать FDR вместо FWER: особенно это касается геномики, протеомики и прочих -ик.

Можно сравнить с обогащением руды: мы отбрасываем мусор и контролируем средний процент "истинных" окрытий.

Самая популярная поправка, которая контролирует FDR -- поправка / процедура Бенджамини-Хохберга. **Процедура предполагает независимость тестовых статистик!**
Через функцию p.adjust() применяем
...
Гистограмма после теста фишера, после применения поправки БХ ушло правее, после Бонферрони еще больше вправо ушло 
...
-   В частотнической статистике можно выделить две оптики: тестирование гипотез и оценивание параметров (конструкция ДИ, интервала совместимости)
...
Что касается FDR:
- Как тест ведет себя в среднем при многократном повторении
...
-   Проводить только те тесты, которые действительно нужны/важны и применять поправки к ним. Не нужно сравнивать все со всем, надо подвести теоритическую базу, обосновать. 
    -   Альтернатива: разумная группировка тестов в "семейства" и применение поправок внутри семейств. Например, сгруппировать тесты по группам пациентов. Например, все, что касается функции сердца или функции почек. 
...
-   Обычно поправки не применяются в safety studies -- исследование побочных или негативных эффектов. Тк ошибка второго рода более смертоносна. 
...
-   FWER как правило обеспечивает более строгий контроль над ошибками первого рода. Его разумно применять в конфирматорных исследованиях
    -   FWER разумно применять, когда нужно проконтролировать все тесты / параметры **вместе**. Пример: бета-блокаторы должны *одновременно* снижать давление и ЧСС. См. также примеры в лекции.
...
1.  *Controlling the False Discovery Rate: A Practical and Powerful Approach to Multiple Testing* by Yoav Benjamini and Yoav Hochberg

2.  *Neural Correlates of Interspecies Perspective Taking in the Post-Mortem Atlantic Salmon: An Argument For Proper Multiple Comparisons Correction* by Craig M. Bennett et al.
